...
@section('content')
    ...
    <div class="row"><button class="btn btn-primary"  style="float:right; margin-bottom:1%; margin-right:1%;" data-toggle="modal" data-target="#addNovaAtividade">Adicionar Atividade</button></div>
        <div class="row">
            <div class="panel panel-primary">
                <div class="panel-heading">CRONOGRAMA DA PROMOÇÃO DE
                @if(isset($cronograma) && count($cronograma) >0)
                 {{date('d/m/Y', strtotime($cronograma[0]->dt_promocao))}}</div>
                 @endif
                            ...
                            @if(isset($cronograma) && count($cronograma) >0)
                                @foreach($cronograma as $c)
                                    <tr>
                                         ...
                                         @if(!empty($c->dt_atividade))
                                         <th style="text-align: center;"><span class="glyphicon glyphicon-ok"></span></th>
                                         @else
                                         <th></th>
                                         @endif
                                        ...
                                        <th>
                                        @if(!empty($c->st_link))
                                            <a href="{{URL($c->st_link.$c->ce_quadroacesso.'/'.$c->id)}}"   class='btn btn-primary fa fa fa-eye' > </a> 
                                        @endif
                                        </th>
                                    </tr>
                                        @endforeach
