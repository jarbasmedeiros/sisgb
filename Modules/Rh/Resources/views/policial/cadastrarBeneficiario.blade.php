...
@section('title', 'SISGP - Cadastro de Beneficiários')
@section('tabcontent')
<div class="tab-pane active" id="cursos">
    <h4 class="tab-title">Cadastro de Beneficiário de Pensão de <span style='text-decoration: underline'>{{ $policial->st_nome}}</span></h4>
    <hr class="separador">
</div>
<div class="modal-content">
  
    <div class="modal-body">
        <form class="form"  method="post" action='{{ url("rh/policiais/cadastrar/" . $policial->id ."/beneficiarios/$idCertidao") }}'>
        {{csrf_field()}}
            <fieldset class="scheduler-border">
                <legend class="scheduler-border">Informações do Beneficiário para Pensão</legend>
                    ...
                    <div class='row'>
                        ...
                        <div style="text-align: center">
                            <div class="form-group">                
                                <a class="btn btn-warning" href="{{url('rh/policiais/edita/'.$policial->id.'/beneficiarios')}}" style='margin-right: 10px;'>
                ...
                @if(isset($beneficiarios) && !empty($beneficiarios))
                <hr>
                <table class="table table-hover">
                    <thead>
                        <tr class="bg-primary">
                            <th colspan = "7">Lista de Beneficiários de {{ $policial->st_nome }} - {{ $policial->st_matricula }}</th>
                        </tr>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $beneficiario->pessoa->st_telefone }}</td>
                            <td>
                            <a href="" onclick="editaBeneficiario(this)" data-idbeneficiario="{{ $beneficiario->id }}" data-idpessoa="{{ $beneficiario->pessoa->id }}" data-cpf="{{ $beneficiario->pessoa->st_cpf }}" data-ordem="{{ $beneficiario->st_ordem }}" data-nome="{{ $beneficiario->pessoa->st_nome }}" data-sexo="{{ $beneficiario->pessoa->st_sexo }}" data-nascimento="{{ $beneficiario->pessoa->dt_nascimento }}" data-mae="{{ $beneficiario->pessoa->st_mae }}" data-telefone="{{ $beneficiario->pessoa->st_telefone }}" data-email="{{ $beneficiario->pessoa->st_email }}" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal-default" title="Editar Beneficiário"><i class="fa fa-fw fa-edit"></i></a> |
                            <a href="{{ route('excluirBeneficiario',['idPolicial'=>$policial->id, 'idDeclaracao'=>$idCertidao, 'idBeneficiario'=>$beneficiario->id]) }}" onclick="return confirm('Você realmente quer excluir este beneficiário desta certidão?');" class="btn btn-danger btn-sm" title="Excluir Beneficiário"><i class="fa fa-fw fa-trash"></i></a></td>
                            <td>
                                @if(is_null($beneficiario->st_caminhoanexo))
                                <button id="{{ $beneficiario->id }}" onclick="mudaBeneficiario(this,1)" data-idbeneficiario="{{ $beneficiario->id }}" data-nome="{{ $beneficiario->pessoa->st_nome }}" data-toggle="modal" data-target="#enviaDocumento" class='btn btn-sm btn-warning' type='button' title='Enviar Documento de Vínculo Familiar'>Documento <i class="fa fa-fw fa-cloud-upload"></i></button>
                                @else
                                <a href="{{ route('downloadDocumentoBeneficiario',['idCertidao'=>$idCertidao, 'idBeneficiario'=>$beneficiario->id, 'tipo'=>'1']) }}" class='btn btn-sm btn-primary' target="_blank" title='Baixar Documento de Vínculo Familiar'>Documento <i class="fa fa-fw fa-download"></i></a>
                                @endif
                                @if(is_null($beneficiario->st_caminhocomprovanteresidencia))
                                <button id="{{ $beneficiario->id }}" onclick="mudaBeneficiario(this,2)" data-idbeneficiario="{{ $beneficiario->id }}" data-nome="{{ $beneficiario->pessoa->st_nome }}" data-toggle="modal" data-target="#enviaDocumento" class='btn btn-sm btn-warning' type='button' title='Enviar Comprovante de Residência'>Comprovante <i class="fa fa-fw fa-cloud-upload"></i></button>
                                @else
                                <a href="{{ route('downloadDocumentoBeneficiario',['idCertidao'=>$idCertidao, 'idBeneficiario'=>$beneficiario->id, 'tipo'=>'2']) }}" class='btn btn-sm btn-primary' target="_blank" title='Baixar Comprovante de Residência'>Comprovante <i class="fa fa-fw fa-download"></i></a>
                                @endif
                            </td>
                        </tr>
                        @endforeach
...
<script src="{{ asset('js/telefone_mascara.js' ) }}"></script>
 ...
 <script>
    ...
    function mudaBeneficiario(componente,tipo){
        ...
        document.getElementById('formulario').action = "/sisgp/rh/policiais/cadastrar/beneficiarios/upload/"+idBeneficiario+"/{{ $idCertidao }}/"+tipo;
