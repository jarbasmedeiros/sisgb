...
@section('js')
    ...
    <script>
        ...
        function buscaHistoricoNotas(id){
            ...
            $.ajax({
                //Enviando via ajax
                url: baseUrl+"boletim/publicar/"+idBoletim,
                data: dados,
                method: 'post',
                error: function(err){
                    $('.overlay').hide();
                    alert("Ocorreu um erro inesperado durante a publicação");
                }
                //Verificando se cadastrou
            }).done(function(response){
                $('.overlay').hide();
                window.location.replace('/sisgp/boletim/lista_boletim_pendente');
            });
        };
            /* ------------------------------------------------------------ */   
